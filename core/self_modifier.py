...
class SelfModifier:
    ...
    def validate_proposal(self, proposal: ModificationProposal) -> list[str]:
        ...
        if proposal.component in self.ALLOWED_TARGETS:
            if proposal.target_name not in self.ALLOWED_TARGETS[proposal.component]:
                errors.append(f"target '{proposal.target_name}' not allowed for {proposal.component}")

        # Check line diff doesn't exceed max
        if proposal.line_diff > MAX_MODIFICATION_LINES:
            errors.append(f"line_diff {proposal.line_diff} exceeds MAX_MODIFICATION_LINES {MAX_MODIFICATION_LINES}")
    ...
    def apply(self, proposal: ModificationProposal) -> ModificationResult:
        ...
        try:
            versions_data = _json.loads(VERSIONS_PATH.read_text()) if VERSIONS_PATH.exists() else {"versions": []}
        ...
        if not target_file.exists():
            return ModificationResult(
                success=False, version_id=version_id, proposal=proposal,
                test_output="", error=f"Target file not found: {target_file}", rolled_back=False
            )
        ...
        try:
            ...
            if proposal.current_value not in content:
                return ModificationResult(
                    success=False, version_id=version_id, proposal=proposal,
                    test_output="", error="current_value not found in target file", rolled_back=False
                )
            ...
            if str(target_file).endswith('.py'):
                ...
                if result.returncode != 0:
                    ...
                    return ModificationResult(
                        success=False, version_id=version_id, proposal=proposal,
                        test_output="", error=f"Syntax error after edit: {result.stderr[:300]}", rolled_back=True
                    )
            ...
            if not canary_ok:
                ...
                return ModificationResult(
                    success=False, version_id=version_id, proposal=proposal,
                    test_output=canary_out, error="Canary test failed", rolled_back=True
                )

            # Log to versions
            self.log_version(proposal, ModificationResult(
                success=True, version_id=version_id, proposal=proposal,
                test_output=canary_out, error="", rolled_back=False
            ))
            ...
            return ModificationResult(
                success=False, version_id=version_id, proposal=proposal,
                test_output="", error=f"Exception during apply: {str(e)[:200]}", rolled_back=True
            )
